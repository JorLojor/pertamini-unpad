...
const CardDashboard = ({ titleCard, dataCard, trendData, idx, activeIdx, onClick }) => {
    ...
    return (
        <div
            ...
            ${activeIdx === idx ? "bg-blue-950 text-white" : "bg-white text-green-950"}`}>
            <div className="flex flex-row justify-between items-center p-4 w-full">
                <p className="text-2xl font-bold text-[22px]">{titleCard}</p>
                <p
                    className={`text-xl font-bold flex items-center ${colorSymbolDesicion(trendData)[0]}`}>
            ...
            <div className="flex flex-row justify-between items-center p-4 w-full">
                <p className={`flex-row font-bold ${aturUkuran(dataCard)} text-5xl`}>
                    {dataCard}
                    <small className="text-3xl">{symbolDesicion()}</small>
                </p>
                <img
                    src={activeIdx === idx ? titleDesicion()[1] : titleDesicion()[0]}
                    alt="Icon"
